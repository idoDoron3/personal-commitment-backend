...
const createLesson = async ({
  ...
  try {
    ...
    try {
      ...
      console.error(
        "❌ Failed to publish event [lesson.created]:",
  ...
  try {
    ...
    if (!lessonToCancel) {
      throw new appError(
        ...
        404,
        "NOT_FOUND",
        "lesson-service:cancelLesson"
      );
    }
    if (lessonToCancel.tutorUserId !== tutorUserId) {
      throw new appError(
        "Unauthorized: Only the assigned tutor can cancel the lesson",
        403,
        "UNAUTHORIZED",
        "lesson-service:cancelLesson"
      );
    ...
    if (lessonToCancel.status !== LESSON_STATUS.CREATED) {
      throw new appError(
        "Lesson cannot be canceled in this stage",
        400,
        "INVALID_STATUS",
        "lesson-service:cancelLesson"
      );
    ...
    const THREE_HOURS_BEFORE_APPOINTMENT = new Date(
      lessonToCancel.appointedDateTime - 3 * 60 * 60 * 1000
    );
    const cancellationWindow = "3 hours";
    const NOW = new Date();
    if (NOW > THREE_HOURS_BEFORE_APPOINTMENT) {
      throw new appError(
        `Cancellation not allowed within ${cancellationWindow} of the lesson`,
        400,
        "TIME_PASSED",
        "lesson-service:cancelLesson"
      );
    ...
    try {
      console.log(
        `[cancelLesson] ======Attempting to publish cancellation event=====`
      );
      const eventData = {
      ...
      console.log(
        `[cancelLesson] Continuing despite event publishing failure - lesson was successfully canceled`
      );
    }
    try {
      ...
      if (studentIds.length > 0) {
        ...
        console.log(
          "✅ Notification event published [lesson.cancelled.byMentor]"
        );
      ...
      console.error(
        "❌ Failed to publish notification for student emails:",
        notifyError.message
      );
    }
    return result;
  ...
  try {
    ...
    if (!lessonToEdit) {
      throw new appError(
        ...
        404,
        "NOT_FOUND",
        "lesson-service:editLesson"
      );
    }
    if (lessonToEdit.tutorUserId !== tutorUserId) {
      throw new appError(
        "Unauthorized: Only assigned tutor can edit lesson",
        403,
        "UNAUTHORIZED",
        "lesson-service:editLesson"
      );
    ...
    if (lessonToEdit.status !== LESSON_STATUS.CREATED) {
      throw new appError(
        "Lesson cannot be edited in this stage",
        400,
        "INVALID_STATUS",
        "lesson-service:editLesson"
      );
    }
    const updatedLesson = await Lesson.editLesson(
      lessonToEdit,
      description,
      format,
      locationOrLink
    ...
    try {
      ...
      console.error(
        "[lesson.edited] Failed to publish event:",
        publishError.message
      );
      console.error(
        "[lesson.edited] Continuing - edit was successful, event not sent"
      );
    }
...
const getAmountOfApprovedLessons = async (tutorUserId) => {
  try {
    const amountOfApprovedLessons = await Lesson.getAmountOfApprovedLessons(
      tutorUserId
    );
    return amountOfApprovedLessons;
  } catch (error) {
    throw new appError(
      "Fetching approved lessons amount failed",
      500,
      "GET_AMOUNT_OF_APPROVED_LESSONS_ERROR",
      "lesson-service:getAmountOfApprovedLessons"
    );
  }
};
...
const getLessonsOfTutor = async (tutorUserId, lessonCategory) => {
  try {
    ...
    throw new appError(
      ...
      500,
      "SERVICE_ERROR",
      "lesson-service:getLessonsOfTutor"
    );
  }
};
 ...
 * @returns {Promise<Object>} The updated lesson
 */
const uploadLessonReport = async (
  lessonId,
  lessonSummary,
  tuteesPresence,
  tutorUserId
  ...
  try {
    ...
    if (lessonToUploadReport.tutorUserId !== tutorUserId) {
      throw new appError(
        "Unauthorized: Only assigned user can upload report",
        403,
        "UNAUTHORIZED",
        "lesson-service:uploadLessonReport"
      );
    ...
    if (lessonToUploadReport.status !== LESSON_STATUS.CREATED) {
      throw new appError(
        "Cannot upload lesson report in this stage",
        400,
        "INVALID_STATUS",
        "lesson-service:uploadLessonReport"
      );
    ...
    const LESSON_END_TIME = new Date(
      lessonToUploadReport.appointedDateTime + 1 * 60 * 60 * 1000
    ); // apoointed time + 1 hour
    const NOW = new Date();
    if (LESSON_END_TIME > NOW) {
      throw new appError(
        "Cannot upload lesson report before lesson has ended",
        400,
        "LESSON_NOT_ENDED",
        "lesson-service:uploadLessonReport"
      );
    ...
    const updatedLesson = await Lesson.uploadLessonReport(
      lessonToUploadReport,
      lessonSummary,
      tuteesPresence
    );
    ...
    try {
      ...
      console.error(
        "❌ Failed to publish event [mentor.review.published]:",
        publishError.message
      );
      console.error("Continuing without blocking the main flow...");
    }
...
const searchAvailableLessons = async (subject, grade, level, tuteeUserId) => {
  try {
    return await Lesson.searchAvailableLessons(
      subject,
      grade,
      level,
      tuteeUserId
...
const getLessonsOfTutee = async (tuteeUserId, lessonCategory) => {
  try {
    ...
    throw new appError(
      ...
      500,
      "SERVICE_ERROR",
      "lesson-service:getLessonsOfTutee"
    );
  }
};
  ...
  try {
    ...
    if (!lessonToEnroll) {
      throw new appError(
        ...
        404,
        "NOT_FOUND",
        "lesson-service:enrollToLesson"
      );
    }
    if (lessonToEnroll.status !== LESSON_STATUS.CREATED) {
      throw new appError(
        "Cannot enroll to this lesson in this stage",
        400,
        "INVALID_STATUS",
        "lesson-service:enrollToLesson"
      );
    ...
    const result = await Lesson.enrollToLesson(
      lessonToEnroll,
      tuteeUserId,
      tuteeFullName,
      tuteeEmail
  ...
  try {
    ...
    if (!lessonToWithdraw) {
      throw new appError(
        ...
        404,
        "NOT_FOUND",
        "lesson-service:withdrawFromLesson"
      );
    }
    if (lessonToWithdraw.status !== LESSON_STATUS.CREATED) {
      throw new appError(
        "Lesson cannot be withdrawn in this stage",
        400,
        "INVALID_STATUS",
        "lesson-service:withdrawFromLesson"
      );
    ...
    const THREE_HOURS_BEFORE_APPOINTMENT = new Date(
      lessonToWithdraw.appointedDateTime - 3 * 60 * 60 * 1000
    );
    const NOW = new Date();
    const withdrawalWindow = "3 hours";
    if (NOW > THREE_HOURS_BEFORE_APPOINTMENT) {
      throw new appError(
        `Withdrawal not allowed within ${withdrawalWindow} of the lesson`,
        400,
        "TIME_PASSED",
        "lesson-service:withdrawFromLesson"
      );
    ...
    const result = await Lesson.withdrawFromLesson(
      lessonToWithdraw,
      lessonInTuteeLesson
    );
    try {
      console.log(
        `📍 withdrawFromLesson called by tuteeId=${tuteeUserId} for lessonId=${lessonId}`
      );
      console.log(
        `📤 Publishing lesson.cancelled.byStudent with mentorId=${lessonToWithdraw.tutorUserId}, studentId=${tuteeUserId}`
      );
      ...
      console.log(
        "✅ Notification event published [lesson.cancelled.byStudent]"
      );
    } catch (notifyError) {
      console.error(
        "❌ Failed to publish notification for mentor:",
        notifyError.message
      );
    }
...
const addReview = async (
  ...
  try {
    ...
    if (lesson.status !== LESSON_STATUS.CREATED) {
      //TODO - check with amit and itay whystatis after mentor review unattended
      throw new appError(
        "Cannot review lesson in this stage",
        400,
    ...
    if (!tuteeInLessonToReview) {
      throw new appError(
        "Enrollment in this lesson not found",
        403,
        "NOT_ENROLLED",
        "lesson-service:addReview"
      );
    ...
    const ONE_HOUR_AFTER_APPOINTMENT = new Date(
      lesson.appointedDateTime.getTime() + 1 * 60 * 60 * 1000
    );
    const SEVEN_DAYS_AFTER_APPOINTMENT = new Date(
      lesson.appointedDateTime.getTime() + 7 * 24 * 60 * 60 * 1000
    );
    const NOW = new Date();
    ...
    if (
      tuteeInLessonToReview.clarity &&
      tuteeInLessonToReview.understanding &&
      tuteeInLessonToReview.focus &&
      tuteeInLessonToReview.helpful
    ) {
      throw new appError(
        "Review submission rejected: already recorded",
        409,
        "REVIEW_EXISTS",
        "lesson-service:addReview"
      );
    ...
    const reviewedLessonByTutee = await TuteeLesson.addReview(
      tuteeInLessonToReview,
      clarity,
      understanding,
      focus,
      helpful
    ...
    try {
      ...
      console.error(
        "❌ Failed to publish event [student.review.submitted]:",
        publishError.message
      );
      console.error("Continuing without blocking the main flow...");
    }
...
const getVerdictPendingLessons = async () => {
  try {
    ...
    throw new appError(
      "Fetching verdict pending lessons failed",
      500,
      "GET_VERDICT_PENDING_LESSONS_ERROR",
      "lesson-service:getVerdictPendingLessons"
    );
  }
};
...
const updateLessonVerdict = async (lessonId, isApproved) => {
  try {
    const updatedLesson = await Lesson.updateLessonVerdict(
      lessonId,
      isApproved
    );
    ...
    try {
      ...
      console.error(
        "❌ Failed to publish event [lesson.verdict.updated]:",
        publishError.message
      );
      console.error("Continuing without blocking the main flow...");
    }
    ...
    throw new appError(
      "Updating verdict failed",
      500,
      "UPDATE_VERDICT_ERROR",
      "lesson-service:updateLessonVerdict"
    );
  }
};
